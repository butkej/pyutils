...
github.com/butkej
twitter: @JoshuaButke
mail: butkej@gmail.com

This submodule contains data preprocessing functions (most often in a [medical] imaging
or computer vision setting).
...
def filter_patches(patch_collection, crop=50, factor=1.0):
    ...
    else: del filtered_patch_collection
...
def central_cutout(img, size=(1000,1000)):
    ...
    cutout = img[x_centroid - int(size[0]/2) : x_centroid + int(size[0]/2), y_centroid - int(size[1]/2) : y_centroid + int(size[1]/2)]
    return cutout
...
def tile_wsi(image, tile_size=(300,300), strides=(150,150)):
        ...
        for i in image:
            ...
            for j in range(int(math.ceil(img_shape[0]/(strides[1] * 1.0)))):
                ...
                for k in range(int(math.ceil(img_shape[1]/(strides[0] * 1.0)))):
                    tile = i[strides[1]*j:min(strides[1]*j+tile_size[1], img_shape[0]), strides[0]*k:min(strides[0]*k+tile_size[0], img_shape[1])]
                    if tile.shape == tile_size:
                        if len(image[0].shape) == 2:
                            tile = np.expand_dims(tile, axis=0) # add channel dimension for torch (only for grayscale images)
                        tile = np.expand_dims(tile, axis=0) # add batch dimension for concat
                        intermediate_result.append(tile)
                    
                if len(inter_intermediate_result) > 1:
                    inter_intermediate_result = np.concatenate(inter_intermediate_result)
    ...
    #    for i in image:
    #        tiles = tf.image.extract_patches(i, ksizes, strides, rates, padding)
    #        tiled_data.append(np.array(tf.reshape(tiles, [-1, input_dim[0], input_dim[1], input_dim[2] ])))
...
def histogram_matching(img, reference, multichannel=False):
    ...
    if multichannel:
        if img.shape[-1] != reference.shape[-1]:
            raise ValueError('Number of channels in input image and reference image do not match!')
        matched = np.empty(img.shape, dtype=img.dtype)
        for channel in range(img.shape[-1]):
            matched_channel = _match_cumulative_cdf(img[..., channel], reference[..., channel])
...
def otsu_thresholding(img, blurring=True):
    ...
    '''
    if blurring == True:
        img = cv2.GaussianBlur(img.astype('uint8'), (5,5), 0) # filters image with a Gaussian 5x5 kernel to remove noise
    _, thresholded_img = cv2.threshold(img.astype('uint8'), 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
...
def filter_contours(contours, hierarchy):
    ...
    for cnt, tree in zip(contours,hierarchy.squeeze()):
        if tree[-1] == [-1]: parent_cnts.append(cnt)

    print(f'After filtering there are now {len(parent_cnts)} contours.')
    return parent_cnts

def find_contours_and_centroids(thresh, morph_kernel=(8,8), lower_bound_area=1000, upper_bound_area=20000):
    ...
    # find contours
    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:] # the cluster opencv version has different returns, thus using [-2:] to get the same outputs
...
def blowup_patches(img, centroids, patch_size=(224,224), multichannel=False):
    ...
    for point in centroids:
        ...
        if (x_left > 0 and y_left > 0) and ( x_right < img.shape[1] and y_right < img.shape[0]):
            #
            # ATTENTION!
            ...
            if not multichannel:
                patch = np.expand_dims(patch, axis=0) # add channel dimension for torch (for grayscale images only)
    ...
    else: del image_patch_collection
...
def build_bags(tiles, labels):
    ...
    if len(result_bags) == len(labels):
        ...
        raise ValueError('Number of Bags is not equal to the number of labels that can be assigned.\nCheck your input data!')
